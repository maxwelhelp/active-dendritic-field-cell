#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""011 — Task-MoE Utility GraphADFC.

Goal: make specialization visible, not just remember previous baselines.

Only compares:
  - always_typed: known strong fixed mixture baseline
  - task_moe_utility: new model with task-conditioned channels + task-conditioned graph subgates
...
def build(name,args):
    ...
    if name=='task_moe_utility': return TaskMoEUtility(args)
...
def train_one(name,task,args,dev,eval_tasks):
    ...
    for step in range(1,args.steps+1):
        ...
        if isinstance(model,TaskMoEUtility) and step>=args.structural_warmup and step%args.structural_every==0: model.graph.structural_step(torch.zeros(4,device=dev), beta=args.utility_beta, cost=args.utility_cost, lr=args.utility_lr)
