...
def train_one(name,task,a,dev):
    ...
    return rows,rows[-1]
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--device',default='cuda'); ap.add_argument('--out',default='runs/v3'); ap.add_argument('--seed',type=int,default=7); ap.add_argument('--steps',type=int,default=180); ap.add_argument('--batch',type=int,default=256); ap.add_argument('--eval-batch',type=int,default=512); ap.add_argument('--seq-len',type=int,default=64); ap.add_argument('--vocab',type=int,default=96); ap.add_argument('--dim',type=int,default=64); ap.add_argument('--depth',type=int,default=2); ap.add_argument('--branches',type=int,default=12); ap.add_argument('--rank',type=int,default=12); ap.add_argument('--mem-dim',type=int,default=64); ap.add_argument('--lr',type=float,default=2e-3); ap.add_argument('--weight-decay',type=float,default=1e-4); ap.add_argument('--log-every',type=int,default=30); ap.add_argument('--tasks',default='mode_select,order_compare,kv_recall4'); ap.add_argument('--models',default='flat_last,mean_pool,adfc2,adfc3'); a=ap.parse_args()
    ...
    print('=== ADFC v3 keyed GPU benchmark ==='); print('gpu',torch.cuda.get_device_name(0),'torch',torch.__version__)
    all_rows=[]; finals=[]
    ...
    winners=[]
    for task in [x for x in a.tasks.split(',') if x]:
        sub=[r for r in finals if r['task']==task]; ad=next((r for r in sub if r['model']=='adfc3'),None); base=max([r for r in sub if r['model']!='adfc3'],key=lambda r:r['best_val_acc'],default=None); win=max(sub,key=lambda r:r['best_val_acc'])
        winners.append({'task':task,'winner':win['model'],'winner_acc':win['best_val_acc'],'adfc3_acc':ad['best_val_acc'] if ad else None,'best_non_adfc3':base['model'] if base else None,'non_adfc3_acc':base['best_val_acc'] if base else None,'adfc3_margin':(ad['best_val_acc'] if ad else 0)-(base['best_val_acc'] if base else 0)})
    ...
    print('\n=== WINNERS ===')
    for w in winners: print(f"{w['task']} winner={w['winner']} adfc3={100*w['adfc3_acc']:.2f}% non_adfc3={w['best_non_adfc3']}:{100*w['non_adfc3_acc']:.2f}% margin={100*w['adfc3_margin']:+.2f}%")
